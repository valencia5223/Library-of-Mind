...
// 두 위도/경도 간 거리(km) 계산 헬퍼 (Haversine 공식)
...
function App() {
  ...
  // 의왕시 기준 좌표 (위치 실패/수동 설정 지원)
  const UIWANG_COORDS = { lat: 37.3447, lng: 126.9683, name: '경기도 의왕시' };

  // 사용자 위치 정보 취득 및 의왕시 수동 설정 지원 함수
  const getUserLocation = useCallback((forceUiwang = false) => {
    if (forceUiwang) {
      setUserLocation(UIWANG_COORDS);
      setIsLocating(false);
      return;
    }
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.warn('Geolocation 위치 취득 실패 또는 거부됨, 의왕시 기준 위치로 설정:', err);
          setUserLocation(UIWANG_COORDS);
          setIsLocating(false);
        },
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    // 사용자 위치 정보가 있으면 각 맛집과의 거리를 계산해 가까운 순(오름차순)으로 정렬
    if (userLocation) {
      return list.map(r => {
        const dist = (r.coords && r.coords.length === 2)
          ? getDistanceKm(userLocation.lat, userLocation.lng, r.coords[0], r.coords[1])
          : Infinity;
    ...
    return list;
  }, [restaurants, selectedMember, selectedRegion, selectedCategory, searchTerm, userLocation]);

  // ──────────────────────────────────────────────────
  // 4-A. 카카오 장소 검색 자동완성 (디바운스 300ms + 현위치 거리순 정렬)
  // ──────────────────────────────────────────────────
  const handleNameInput = useCallback((value) => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      ps.keywordSearch(value, (data, status) => {
        console.log('[자동완성] 거리순 검색 결과:', status, data?.length, data);
        if (status === window.kakao.maps.services.Status.OK) {
          setNameSuggestions(data.slice(0, 8));
  ...
  return (
    <div className="bistro-app">
      ...
      <section className="search-filter-section">
        ...
        {/* 내 위치 기반 가까운 순 정렬 상태 노출 배지 */}
        <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', background: '#f0f9ff', border: '1px solid #bae6fd', padding: '8px 12px', borderRadius: '8px', fontSize: '0.82rem', color: '#0369a1' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ fontWeight: 800 }}>📍 기준 위치:</span>
            <span style={{ fontWeight: 700, color: '#0284c7', background: '#e0f2fe', padding: '2px 8px', borderRadius: '12px' }}>
              {userLocation ? (userLocation.name || '내 현재 위치') : '위치 파악 중...'}
            </span>
            <span style={{ color: '#0369a1', fontSize: '0.78rem' }}>(가까운 맛집 순으로 자동 정렬)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <button
              type="button"
              onClick={() => getUserLocation(true)}
              style={{ background: '#ffffff', color: '#0284c7', border: '1px solid #38bdf8', borderRadius: '6px', padding: '4px 8px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
              title="경기도 의왕시 좌표(37.3447, 126.9683)로 위치를 설정합니다"
            >
              📍 의왕시로 설정
            </button>
            <button
              ...
              disabled={isLocating}
              style={{ background: '#0284c7', color: '#ffffff', border: 'none', borderRadius: '6px', padding: '4px 10px', fontSize: '0.75rem', fontWeight: 700, cursor: isLocating ? 'not-allowed' : 'pointer' }}
            >
              {isLocating ? '위치 측정 중...' : '🔄 GPS 재측정'}
            </button>
          </div>
        </div>
      </section>
                  ...
                  <div key={rest.id} className="bistro-card" onClick={() => setSelectedRestaurant(rest)}>
                    <div className="card-header">
                      <div className="flex align-center gap-1">
                        ...
                        {rest.distance !== undefined && rest.distance !== Infinity && (
                          <span className="distance-tag" style={{ background: '#e0f2fe', color: '#0369a1', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 700 }}>
                            📍 {rest.distance < 1 ? `${Math.round(rest.distance * 1000)}m` : `${rest.distance.toFixed(1)}km`}
                          </span>
                        )}
                      </div>
