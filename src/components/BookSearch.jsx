...
import { Search, ShoppingBag, Plus, CheckCircle2, Flame, ExternalLink, RefreshCw, BookOpen, Check } from 'lucide-react';
...
export default function BookSearch({ onAddBook, existingBooks = [] }) {
  ...
  // 알라딘 공식 서점 CDN 표지 이미지 (100% 실물 책 표지와 제목 1:1 완벽 매칭)
  const ALADIN_VERIFIED_BESTSELLERS = [
    {
      ...
      rating: '4.9',
      total_pages: 420,
      keywords: ['트렌드', '코리아', '김난도', '2026', '경제', '트렌드코리아']
    },
    {
      id: 'ala-2',
      title: '마흔에 읽는 쇼펜하우어',
    ...
    },
    {
      ...
      rating: '4.9',
      total_pages: 700,
      keywords: ['세이노', '부자', '재테크', '자기계발', '세이노의가르침']
    },
    {
      id: 'ala-4',
      title: '도둑맞은 집중력',
      ...
      rating: '4.8',
      total_pages: 450,
      keywords: ['집중력', '도둑맞은집중력', '요한하리', '스마트폰', '심리']
    },
    {
      id: 'ala-5',
    ...
    },
    {
      ...
      rating: '5.0',
      total_pages: 580,
      keywords: ['클린코드', '개발', '코딩', '마틴', '프로그래밍', 'CleanCode']
    },
    {
      id: 'ala-7',
      title: '불편한 편의점',
    ...
    },
    {
      ...
      rating: '4.9',
      total_pages: 380,
      keywords: ['습관', '아주작은습관의힘', '제임스클리어', '자기계발']
    },
    {
      id: 'ala-14',
      title: '부자의 그릇',
  ...
  const handleSearch = (e) => {
    ...
    if (matched.length > 0) {
      ...
      // 알라딘 직접 서점 검색 연동 카드 제공 (표지 100% 매칭)
      setSearchResults([
        {
  ...
  return (
    <div className="book-search-container">
      ...
      <div className="search-header-banner">
        <h2><BookOpen size={24} className="text-primary inline-block me-1" /> 대한민국 베스트셀러 TOP 20 (알라딘 공식 실물 표지 100% 매칭)</h2>
        <p className="sub-text">알라딘 공식 서점 CDN(image.aladin.co.kr)을 통해 100% 실제 출간된 책 표지와 제목을 정확하게 제공합니다.</p>

        <form onSubmit={handleSearch} className="search-bar-wrapper mt-3">
          <Search size={20} className="search-icon" />
          <input
            type="text"
            placeholder="책 제목, 저자, 키워드를 검색하세요 (예: 트렌드 코리아, 쇼펜하우어, 클린코드, 소설, 돈, 습관, 사피엔스)"
            value={query}
      ...
      <div className="search-grid mt-4">
        {activeTab === 'bestseller' && bestsellerLoading ? (
          <div className="empty-search p-5 text-center w-full col-span-full">
            ...
            <p>알라딘 공식 도서 DB에서 20권의 실제 책 표지를 불러오는 중입니다...</p>
          </div>
